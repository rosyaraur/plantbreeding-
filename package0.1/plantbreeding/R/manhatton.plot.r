manhatton.plot <- function (dataframe, SNPname, chromosome, position, pvcol, ylabel = "pvalue", pconv= "-log10", 
  ymax = "maximum", ymin = "minimum", gapbp = 500, pch = c(18, 19, 20), color = c("midnightblue", "lightpink4", "blue"), 
  line1, line2){
  dat <- data.frame(dataframe[, SNPname], dataframe[, chromosome], 
  dataframe[, position], dataframe[, pvcol])
  names(dat) <- c("SNP", "chr", "pos", "pval")
  dat = subset(na.omit(dat[order(dat$chr, dat$pos), ]), (dat$pval >  0 & dat$pval <= 1))
  if(pconv == "-log10"){ 
